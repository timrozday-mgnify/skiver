 ...
 */
pub fn _get_neighbors(value: u64, value_size: u8) -> FxHashMap<u64, NeighborInfo> {
    // get all the values with edit distance 1 from the input value

    let mut neighbors: FxHashMap<u64, NeighborInfo> = FxHashMap::default();
...
pub fn estimate_c_from_raw_files(files: &[&str]) -> (usize, u64) {
    ...
    let total_size: u64 = files
        ...
        .map(|f| {
            ...
            if f.to_lowercase().ends_with(".gz") {
                size * GZ_FACTOR
            } else {
                size
            }
