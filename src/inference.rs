...
/// Bootstrap confidence intervals are reported at the 5th and 95th percentiles.
const CI_LOWER_PERCENTILE: f32 = 0.05;
const CI_UPPER_PERCENTILE: f32 = 0.95;
...
impl ErrorAnalyzer {
    pub fn new(args: AnalyzeArgs) -> Self {
        let method = match args.estimation_method.as_str() {
            ...
            _ => {
                panic!(
                    "Unknown ratio estimation method: {}. Supported methods are: slope, linear_fit, ratio_mean, sum_ratio.",
    ...
    fn random_subsample_with_replacement(x: &Vec<usize>, n: usize) -> Vec<usize> {
        ...
        (0..n).map(|_| x[rng.random_range(0..x.len())]).collect()
    }
    ...
    fn fit_hazard_ratio_weibull_distribution_cloglog(
        &self,
        hazard_ratios: &Vec<f32>,
    ) -> (f32, f32) {
        ...
        let y = hazard_ratios
            .iter()
            .map(|&hr| (-(-hr.clamp(EPSILON, 1.0 - EPSILON)).ln_1p()).ln())
            .collect::<Vec<f32>>();
    ...
    fn fit_hazard_ratio(&self, hazard_ratios: &Vec<f32>) -> (f32, f32) {
        match self.args.hazard_model.as_str() {
            ...
            _ => {
                panic!(
                    "Unknown hazard model: {}. Supported models are: weibull, constant.",
                    self.args.hazard_model
                );
            }
        }
    }
    ...
    fn binomial_cdf_lower(n: u32, k_obs: u32, p: f64) -> f64 {
        ...
        if p >= 1.0 {
            return if k_obs >= n { 1.0 } else { 0.0 };
        }
        let mean = n as f64 * p;
        let var = n as f64 * p * (1.0 - p);
        if var < 1e-10 {
            return if k_obs as f64 >= mean { 1.0 } else { 0.0 };
        }
    ...
    pub fn find_hazard_ratio_outliers(&self, stats: &KVmerStats) -> Vec<usize> {
        ...
        for iter in 0..max_iter {
            ...
            if (lambda - prev_lambda).abs() < convergence_tol
                && (beta - prev_beta).abs() < convergence_tol
            {
                info!(
                    "Iterative outlier removal converged after {} iteration(s).",
                    iter
                );
                break;
            }
            ...
            for v in 1..=v_max {
                ...
                let t1_beta = if t > 1.0 {
                    (t - 1.0).powf(beta as f64)
                } else {
                    0.0
                };
        ...
        let indices: Vec<usize> = (0..n_keys).filter(|&i| active[i]).collect();
        info!(
            "Identified {} inliers out of {} data points based on iterative Binomial outlier removal ({}%).",
            indices.len(),
            n_keys,
            (indices.len() as f32 / n_keys as f32) * 100.0
        );
        indices
    }
    ...
    pub fn estimate_error_rate(
        ...
        stats: &KVmerStats,
        indices: &Vec<usize>,
    ) -> HashMap<(EditOperation, u8, u8), u32> {
        ...
        for op in operations.iter() {
            ...
            let lower = estimates_op[(n as f32 * CI_LOWER_PERCENTILE) as usize];
            let upper = estimates_op[(n as f32 * CI_UPPER_PERCENTILE) as usize];
     ...
     */
    pub fn estimate_hazard_ratio_confidence_interval(
        &self,
        stats: &KVmerStats,
        indices: &Vec<usize>,
    ) -> ((f32, f32), (f32, f32), Vec<(f32, f32)>, (f32, f32)) {
        let mut x: &Vec<u32>;
        let mut y: &Vec<u32>;
        ...
        for _ in 0..self.args.num_experiments {
            let indices_sample =
                Self::random_subsample_with_replacement(indices, indices.len() as usize);
        ...
        let lower_lambda = lambda_list[(self.args.num_experiments as f32 * CI_LOWER_PERCENTILE) as usize];
        let upper_lambda = lambda_list[(self.args.num_experiments as f32 * CI_UPPER_PERCENTILE) as usize];

        beta_list.sort_by(f32::total_cmp);
        let lower_beta = beta_list[(self.args.num_experiments as f32 * CI_LOWER_PERCENTILE) as usize];
        let upper_beta = beta_list[(self.args.num_experiments as f32 * CI_UPPER_PERCENTILE) as usize];
        ...
        for v in 0..hazard_ratio_list.len() {
            ...
            let h_lower = hazard_ratio_list[v][(self.args.num_experiments as f32 * CI_LOWER_PERCENTILE) as usize];
            let h_upper = hazard_ratio_list[v][(self.args.num_experiments as f32 * CI_UPPER_PERCENTILE) as usize];
            hazard_ratio_range_list.push((h_lower, h_upper));
        }

        error_rate_list.sort_by(f32::total_cmp);
        let lower_error_rate = error_rate_list[(self.args.num_experiments as f32 * CI_LOWER_PERCENTILE) as usize];
        let upper_error_rate = error_rate_list[(self.args.num_experiments as f32 * CI_UPPER_PERCENTILE) as usize];

        (
            (lower_lambda, upper_lambda),
            (lower_beta, upper_beta),
            hazard_ratio_range_list,
            (lower_error_rate, upper_error_rate),
        )
    }
    ...
    pub fn estimate_hazard_ratio(
        ...
        stats: &KVmerStats,
        indices: &Vec<usize>,
    ) -> (f32, f32, Vec<f32>, Vec<u32>, Vec<u32>) {
        let mut x: &Vec<u32>;
        let mut y: &Vec<u32>;
    ...
    pub fn key_coverage(&self, stats: &KVmerStats, indices: &Vec<usize>) -> (f32, (f32, f32)) {
        let mut coverages: Vec<u32> = indices
            .iter()
            .map(|&i| stats.error_summary.total_counts[i])
            .collect();
        ...
        let coverage_ci_lower = coverages[(n as f32 * CI_LOWER_PERCENTILE) as usize] as f32;
        let coverage_ci_upper = coverages[(n as f32 * CI_UPPER_PERCENTILE) as usize] as f32;

        (median_coverage, (coverage_ci_lower, coverage_ci_upper))
    }

    pub fn estimate_true_coverage(
        &self,
        estimated_lambda: f32,
        estimated_beta: f32,
        key_coverage: (f32, (f32, f32)),
    ) -> (f32, (f32, f32)) {
        // estimate survival rate at k
        let survival_rate: f32 =
            (-estimated_lambda * ((self.args.k as f32).powf(estimated_beta))).exp();
        ...
        (
            estimated_coverage,
            (estimated_coverage_ci_lower, estimated_coverage_ci_upper),
        )
    }
    ...
    pub fn calibrate_qscores(&self, stats: &KVmerStats) -> Vec<QscoreCalibration> {
        ...
        let mut qscores: Vec<u8> = qscore_correct
            .keys()
        ...
        let mut bootstrap_rates: HashMap<u8, Vec<f64>> =
            qscores.iter().map(|&q| (q, Vec::new())).collect();
        for _ in 0..self.args.num_experiments {
        ...
        for &q in &qscores {
            ...
            let n = rates.len();
            let lower = if n > 0 {
                rates[(n as f64 * CI_LOWER_PERCENTILE as f64) as usize]
            } else {
                0.0
            };
            let upper = if n > 0 {
                rates[((n as f64 * CI_UPPER_PERCENTILE as f64) as usize).min(n - 1)]
            } else {
                0.0
            };
    ...
    pub fn analyze(&self, stats: &KVmerStats) -> ErrorSpectrum {
        ...
        let (lambda, beta, hazard_ratio, x_sum, y_sum) =
            self.estimate_hazard_ratio(stats, &indices);
        let (lambda_ci, beta_ci, hazard_ratio_ci, error_rate_ci) =
            self.estimate_hazard_ratio_confidence_interval(stats, &indices);
        ...
        if let Some(prefix) = &self.args.output_prefix {
            ...
            let file = File::create(&hazard_ratio_output)
                .expect("Could not create hazard ratio output file.");
            let mut writer = BufWriter::new(file);

            writeln!(
                writer,
                "t,num_candidates,num_survival,hazard_ratio,5th_percentile,95th_percentile"
            )
            ...
            fs::write(
                format!("{}.kvmer.csv", prefix),
                stats.error_summary.to_csv(Some(&indices)),
            )
            .unwrap();
            fs::write(
                format!("{}.summary_error_spectrum.csv", prefix),
                stats.error_spectrum.to_csv(Some(&indices)),
            )
            .unwrap();
            fs::write(
                format!("{}.summary_error_spectrum_dependence_on_t.csv", prefix),
                stats.error_spectrum.to_dependence_on_t_csv(
                    Some(&indices),
                    self.args.k as usize,
                    self.args.ignore_last_hazard_ratios,
                ),
            )
            .unwrap();
            fs::write(
                format!("{}.summary_phred.csv", prefix),
                stats.phred_summary.to_csv(Some(&indices)),
            )
            .unwrap();
            fs::write(
                format!("{}.summary_read_position.csv", prefix),
                stats.read_position_summary.to_csv(Some(&indices)),
            )
            .unwrap();
        }
...
pub fn spectrum_to_str(spectrum: &ErrorSpectrum, bidirectional: bool) -> String {
    ...
    result.push_str(&format!(
        "{:.6},{:.6}~{:.6},",
        spectrum.per_base_error_rate.0,
        (spectrum.per_base_error_rate.1).0,
        (spectrum.per_base_error_rate.1).1
    ));
    result.push_str(&format!(
        "{:.6},{:.6}~{:.6},",
        spectrum.effective_error_rate.0,
        (spectrum.effective_error_rate.1).0,
        (spectrum.effective_error_rate.1).1
    ));

    // hazard ratio parameters a and b
    result.push_str(&format!(
        "{:.6},{:.6}~{:.6},",
        spectrum.estimated_lambda.0,
        (spectrum.estimated_lambda.1).0,
        (spectrum.estimated_lambda.1).1
    ));
    result.push_str(&format!(
        "{:.6},{:.6}~{:.6},",
        spectrum.estimated_beta.0,
        (spectrum.estimated_beta.1).0,
        (spectrum.estimated_beta.1).1
    ));

    // key coverage and estimated true coverage
    result.push_str(&format!(
        "{:.6},{:.6}~{:.6},",
        spectrum.key_coverage.0,
        (spectrum.key_coverage.1).0,
        (spectrum.key_coverage.1).1
    ));
    result.push_str(&format!(
        "{:.6},{:.6}~{:.6},",
        spectrum.estimated_coverage.0,
        (spectrum.estimated_coverage.1).0,
        (spectrum.estimated_coverage.1).1
    ));
