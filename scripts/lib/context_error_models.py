...
def _aggregate_one_prefix_screen(
...
) -> tuple[dict[int, "torch.Tensor"], dict[int, int], dict[int, int], int, int] | None:
    ...
    return count_tensors, file_totals, file_skipped_by_length, file_total, file_skipped
...
def aggregate_context_length_screen_counts(
    ...
    *,
    ...
    def accumulate(result) -> None:
        ...
        if result is None:
            return
        file_tensors, file_totals, file_skipped_by_length, file_total, file_skipped = result
    ...
    if parallel:
        ...
        with concurrent.futures.ProcessPoolExecutor(max_workers=max_workers) as pool:
            futures = {
                pool.submit(
                    _aggregate_one_prefix_screen, prefix, list(context_lengths), passes_filter_only
                ): prefix
...
def empty_guide(
    ...
    del counts, init_logits, run_values, additive_context, context_indices
...
def train_counts(
    ...
    *,
    ...
    for step in range(num_steps):
        loss = float(svi.step(counts, init_logits, run_values, additive_context, context_indices))
        losses.append(loss)
        if progress_callback is not None:
...
def train_bayesian_counts(
    ...
    *,
    ...
    for step in range(num_steps):
        loss = float(
            svi.step(counts, init_logits, run_values, prior_scale, additive_context, context_indices)
        )
        losses.append(loss)
        if progress_callback is not None:
            progress_callback(step, loss)
...
def elbo_loss(
    ...
    *,
    ...
    loss = -log_likelihood(counts, params, run_values, additive_context, context_indices)
...
def compute_marginal_error_rate(
    ...
    """
    logits = _get_full_logits(params, counts, run_values, additive_context, context_indices)
...
def num_free_parameters(
    ...
    *,
    ...
    if additive_context:
        ...
        return (1 + (NUM_CONTEXT_BASES - 1) * context_length) * (
            NUM_ERROR_TYPES - 1
        )
